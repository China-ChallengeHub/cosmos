...
class RobertaForMultipleChoice_Fix_Head(BertPreTrainedModel):
    # Commonsense_Mask + Dependency_Mask + Entity_Mask + Ner_Mask
    def __init__(self, config):
    ...
    def forward(self,
        ...
        ner_mask    =    ner_mask.unsqueeze(2).expand(batch_size, num_choices, seq_len, seq_len)
        ...
        flat_ner_mask         = ner_mask.view(        -1, seq_len, seq_len)
        ...
        flat_ner_mask         = flat_ner_mask.view(        -1, 1, seq_len, seq_len)
        ...
        slf_attn_mask = torch.cat(
            [flat_commonsense_mask] * 3 + [flat_dependency_mask] * 3 +
            [flat_entity_mask] * 3 + [flat_ner_mask] * 3, dim=1)
...
class RobertaForMultipleChoice_Fusion_All(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        flat_mask = torch.cat([flat_commonsense_mask, flat_dependency_mask, flat_entity_mask, flat_ner_mask], dim=1)
...
class RobertaForMultipleChoice_Fusion_Head(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        # flat_mask: (batch_size * choice_num) * 4 * seq_len * seq_len
        flat_mask = torch.cat(
            [flat_commonsense_mask, flat_dependency_mask, flat_entity_mask, flat_ner_mask], dim=1)

        # flat_mask: (batch_size * choice_num) * 12 * 4 * seq_len * seq_len
