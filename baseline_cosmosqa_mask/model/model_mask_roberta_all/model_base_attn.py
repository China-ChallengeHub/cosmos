...
class RobertaForMultipleChoice_Fusion_All(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.n_head = 12
        self.hidden_size = int(config.hidden_size)
        self.layer_size = int(self.hidden_size / (self.n_layer * self.n_head))
        self.seq_len = 256
        ...
        self.linear = nn.Linear(self.hidden_size, self.n_layer * self.n_head * 4)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self,
        ...
        # weight: (batch_size * choice_num) * (n_layer * n_head * 4)
        weight = self.linear(weight)

        # debug: 可能需要加上一个FC层

        # weight: (batch_size * choice_num) * n_layer * n_head * 4
        weight = weight.view(batch_size * num_choices, self.n_layer, self.n_head, 4)

        # weight: n_layer * (batch_size * choice_num) * n_head * 4
        ...
        # weight: n_layer * (batch_size * choice_num) * n_head * 4 * 1 * 1
        weight = weight.unsqueeze(-1).unsqueeze(-1)
        ...
        # flat_mask: n_layer * (batch_size * choice_num) * n_head * 4 * 1 * 1
        flat_mask = flat_mask.unsqueeze(2).repeat(1, 1, self.n_head, 1, 1, 1)

        # flat_mask: n_layer * (batch_size * choice_num) * n_head * 4 * seq_len * seq_len
        flat_mask = torch.mul(weight.float(), flat_mask.float())
...
class RobertaForMultipleChoice_Fusion_Head_Bert_Self_Attn(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaModel(config)
        self.transformer_mrc = Trans_Encoder_self_attn(n_layers=3, n_head=12, d_k=64, d_v=64,
                                                       d_model=768, d_inner=4096, dropout=0.1)
